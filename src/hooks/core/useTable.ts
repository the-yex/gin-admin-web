...
function useTableImpl<TApiFn extends (params: any) => Promise<any>>(
  ...
  const sizeKey = paginationKey?.page_size || tableConfig.paginationKey.page_size
    ...
    try {
      ...
      if (useCache && cache) {
        ...
        if (cachedItem) {
          ...
          if (paramsRecord[sizeKey] !== pagination.page_size) {
            paramsRecord[sizeKey] = pagination.page_size
          }
      ...
      if (err instanceof Error && err.message === '请求已取消') {
        ...
        return { records: [], total: 0, pahe: 1, page_size: 10 }
      }
  ...
  const resetSearchParams = async (): Promise<void> => {
    ...
    // 重置分页
    pagination.page = 1
    pagination.page_size = defaultPagination[sizeKey] as number
