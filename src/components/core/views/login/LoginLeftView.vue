...
<style lang="scss" scoped>
  ...
  .login-left-view {
    ...
    padding: 15px;
    overflow: hidden;
    background: linear-gradient(135deg, $primary-light-9 0%, $primary-light-8 50%, $primary-light-7 100%);
    ...
    .particle {
      position: absolute;
      background: radial-gradient(circle, rgba($primary-base, 0.6), rgba($primary-base, 0.2));
      border-radius: 50%;
    ...
    .left-img {
      ...
      filter: drop-shadow(0 10px 30px rgba($primary-base, 0.25));
    }
    ...
    // 为特定元素添加呼吸效果
    .bg-bubble {
      animation: scaleIn 1.2s ease forwards, breathe 6s ease-in-out infinite;
    }
